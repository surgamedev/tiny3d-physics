...
{
    if (camera->distance_from_barycenter < camera->settings.target_zoom) 
    ...
    if (camera->offset_angle > camera->settings.target_offset) 
...
{
    if (camera->distance_from_barycenter > camera->settings.target_zoom_aim) 
    ...
    if (camera->offset_angle < camera->settings.target_offset_aim) 
        camera->offset_acceleration = camera->settings.offset_acceleration_rate * (camera->settings.offset_max_speed  - camera->offset_speed);
